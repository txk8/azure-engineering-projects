...
# Step 3 - Enable Stati Web hosting
In the Azure Portal:

Go to your storage account → Data management → Static website
Toggle to Enabled
Set Index document name: index.html
Set Error document path: 404.html
Save — note the Primary endpoint URL
...
az storage blob upload \
  ...
  Grab the DNS validation token by running this CLI:
...
TypeHostnameValueCNAMEwww<YOUR_ENDPOINT>.z03.azurefd.netTXT_dnsauth.www<VALIDATION_TOKEN_FROM_ABOVE>

  # Step 11 - Limk Custom Domain to Route
  CLI:
  az afd route update \
  --route-name <YOUR_ROUTE_NAME> \
  --profile-name <YOUR_FD_PROFILE> \
  ...
  Check Validaion running this CLI:
  az afd custom-domain show \
  --custom-domain-name <YOUR_CUSTOM_DOMAIN_NAME> \
  --profile-name <YOUR_FD_PROFILE> \
  --resource-group <YOUR_RESOURCE_GROUP> \
  ...
  # Cleanup - 
  You can delete the resource group in the portal, or run this CLI:
  az group delete \
  --name <YOUR_RESOURCE_GROUP> \
  --yes --no-wait

  This will delete all resources used, assuming you have placed them all in the same resource group.
  # Issues Encountered -
